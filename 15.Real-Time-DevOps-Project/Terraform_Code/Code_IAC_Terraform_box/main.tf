...
resource "aws_instance" "ec2" {
  ...
  key_name               = "ssh_keypair" # Change key name as per your setup
...
resource "aws_instance" "terraform_vm" {
  ami                    = data.aws_ami.ubuntu.id
  key_name               = "ssh_kaypair" # Change key name as per your setup
  ...
  provisioner "file" {
    ...
    connection {
      type        = "ssh"
      user        = "ubuntu"
      private_key = file("ssh_kaypair.pem")
      host        = self.public_ip
    }
  }
  ...
  provisioner "remote-exec" {
    ...
    connection {
      type        = "ssh"
      user        = "ubuntu"
      private_key = file("ssh_keypair.pem")
      host        = self.public_ip
    }
  }
}
